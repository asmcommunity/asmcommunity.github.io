...
[ORG 0000h]	;Assembler Directive denoting this as existing at offset 0000h
...
gdtr:
gdtsze dw 00C0h		;size of GDT is C0h (192 bytes), each descriptor is 64-bytes long
...
code	equ $-gdt  	;code segment with 4GB flat memory model
	...
	db 0x9B		;first byte of attribute flags: (present,ring0,predef,nonconforming,read/execute,accessed)
...
boot:
	...
	mov ax, 7BFFh		;stack grows down from 7BFFh
    	mov ss, ax

    	mov sp, 0200h		;stack size is 200h bytes
